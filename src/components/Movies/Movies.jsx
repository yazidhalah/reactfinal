...
export default function Movies() {
    ...
    return (
        <>
            {!isLoading && isSuccess ? (
                <div className={`container mt-5 ${styles.scrollcontainer}`}>
                    ...
                    <div className="row">
                        {movieList.length > 0 ? (
                            movieList.map((mov, index) => (
                                <div
                                    key={index}
                                    className="col-lg-3 col-md-4 col-sm-6 col-12 my-4 text-center"
                                    onClick={() => openDetailsModal(mov)}
                                >
                                    <div className={styles.moviecard}>
                                        <div className={styles.moviedetails}>
                                            <h2 className={styles.movietitle}>{mov.title}</h2>
                                            <p className={styles.movierating}>{mov.overview}</p>
                                        </div>
                                        <span className={styles.ratebutton}>
                                            {(parseFloat(mov.vote_average) * 10).toFixed(2)}
                                        </span>
                                        <img
